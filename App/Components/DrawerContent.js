...
import { View, StyleSheet,Image,Text, TouchableOpacity,Alert, Platform, StatusBar} from 'react-native';
import {
...
import DeviceInfo from 'react-native-device-info';
  ...
  export const DrawerContent = inject("AuthStore", "CompaignsStore", "SettingsStore")(observer((props) =>{

      
   const {firstName, lastName, userImage, userName ,isLogin,selectedMenuItem} = props.AuthStore
      ...
      <View style = {{flex: 1}}>
        {/* <StatusBar backgroundColor='red' barStyle="dark-content"/> */}
          ...
          <DrawerContentScrollView {...props}>
                ...
                <View style = {{flex: 1}}>
                    <TouchableOpacity style = {{...styles.userInfo, alignItems: 'center'}} onPress={()=>{isLogin?props.navigation.navigate('ProfileStackScreen'):props.navigation.navigate('AuthStack')}}>
                        <Avatar.Image 
                            source = {imageUrl}
                            size={70}
                        ...
                        />
                        <View style = {{paddingLeft: metrics.dimen_15, justifyContent: 'space-between'}}>
                            <Text numberOfLines={2} style = {{...styles.menuTextStyle,textTransform: 'capitalize',width:metrics.dimen_120}}>
                                {isLogin?(firstName + " " + lastName):strings('Guest')}
                                </Text>
                            {isLogin&&<Caption style={{color: 'black'}}>@{userNamenew}</Caption>}
                           {!isLogin&& <Text style = {{...styles.menuTextStyle,textTransform: 'uppercase',color:colors.app_Blue,marginTop:2,fontSize:metrics.dimen_12}}>{strings('Login')}</Text>}

                        </View>
                        <Image source={images.rightArrowIcon} style = {{marginLeft: metrics.dimen_50,tintColor: 'lightgray',marginRight:metrics.dimen_10}}/>
                    </TouchableOpacity>
                    <Drawer.Section style = {styles.drawerSection}>
                        <DrawerItem
                            focused={selectedMenuItem === 0 ? true : false}
                            activeBackgroundColor={colors.menuActiveColor}
                            icon = {() => renderDrawerImage(images.home,selectedMenuItem === 0 ? true : false)}
                            label={()=>renderLabel(strings('Home'),selectedMenuItem === 0 ? true : false)}
                            onPress={()=>
                                {
                        ...
                        <DrawerItem
                            ...
                            activeBackgroundColor={colors.menuActiveColor}
                            icon = {() => renderDrawerImage(images.myCompaign,selectedMenuItem === 1 ? true : false)}
                            label={()=>renderLabel(strings('MyCampaign'),selectedMenuItem === 1 ? true : false)}
                            onPress={()=>
                                {
                                   // props.AuthStore.setSelectedMenuItem(isLogin ? 0 : 0)
                                    props.AuthStore.setSelectedMenuItem(0)
                                   // selectedMenuItem=isLogin ? 1 : 0
                                    isLogin ? props.navigation.navigate('MyCompaign',{screen: 'MyCompaign'}) :
                         ...
                         <DrawerItem
                            ...
                            activeBackgroundColor={colors.menuActiveColor}
                            icon = {() => renderDrawerImage(images.myApplications,selectedMenuItem === 2 ? true : false)}
                            label={()=> myApplicationsList.length>0 ? 
                                renderLabelMyApplications(strings('My_Applications'),myApplicationsList,selectedMenuItem === 2 ? true : false) : 
                                renderLabel(strings('My_Applications'),selectedMenuItem === 2 ? true : false)
                        }
                            onPress={()=>
                                {
                         ...
                         <DrawerItem
                            ...
                            activeBackgroundColor={colors.menuActiveColor}
                            icon = {() =>renderDrawerImage(images.aboutKoli,selectedMenuItem === 3 ? true : false)}
                            label={()=>renderLabel(strings('AboutKoli'),selectedMenuItem === 3 ? true : false)}
                            onPress={()=>{
                         ...
                         <DrawerItem
                            ...
                            activeBackgroundColor={colors.menuActiveColor}
                            icon = {() => renderDrawerImage(images.privacyPolicy,selectedMenuItem === 4 ? true : false)}
                            label={()=>renderLabel(strings('PrivacyPolicy'),selectedMenuItem === 4 ? true : false)}
                            onPress={()=>{
                         ...
                         <DrawerItem
                            ...
                            activeBackgroundColor={colors.menuActiveColor}
                            icon = {() => renderDrawerImage(images.termsCondition,selectedMenuItem === 5 ? true : false)}
                            label={()=>renderLabel(strings('TermsConditions'),selectedMenuItem === 5 ? true : false)}
                            onPress={()=>{
                         ...
                         <DrawerItem
                            ...
                            activeBackgroundColor={colors.menuActiveColor}
                            icon = {() => renderDrawerImage(images.faq,selectedMenuItem === 6 ? true : false)}
                            label={()=>renderLabel(strings('Faqs'),selectedMenuItem === 6 ? true : false)}
                            onPress={()=>{
                         ...
                         <DrawerItem
                            ...
                            activeBackgroundColor={colors.menuActiveColor}
                            icon = {() => renderDrawerImage(images.help,selectedMenuItem === 7 ? true : false)}
                            label={()=>renderLabel(strings('Help'),selectedMenuItem === 7 ? true : false)}
                            onPress={()=>
                                {
                                    //props.AuthStore.setSelectedMenuItem(isLogin ? 7 : 0)
                                    props.AuthStore.setSelectedMenuItem(7)
                                    //selectedMenuItem=isLogin ? 7 : 0
                                    isLogin?props.navigation.navigate('HelpStackScreen'):props.navigation.navigate('AuthStack')
                                }
                            }
                         />
                         <DrawerItem
                            ...
                            activeBackgroundColor={colors.menuActiveColor}
                            icon = {() =>renderDrawerImage(images.setting,selectedMenuItem === 8 ? true : false)}
                            label={()=>renderLabel(strings('Setting'),selectedMenuItem === 8 ? true : false)}
                            onPress={()=>
                                {
                                    //props.AuthStore.setSelectedMenuItem(isLogin ? 8 : 0)
                                    props.AuthStore.setSelectedMenuItem(8)
                                    //selectedMenuItem=isLogin ? 8 : 0
                                    isLogin?props.navigation.navigate('SettingStackScreen'):props.navigation.navigate('AuthStack')
                                }
                            }
            ...
            <Drawer.Section style = {styles.bottomDrawerSection}>
                <Text style = {{alignSelf: 'center', color: 'rgba(54,54,54,1)', fontSize: metrics.text_medium, fontWeight: metrics.LatoRegular, fontFamily: metrics.Lato}}>{strings('version_code')+" " + DeviceInfo.getVersion() + ' ( ' + DeviceInfo.getBuildNumber() + ' )'	}</Text>
            </Drawer.Section>
      </View>
      )
  })      
...
const renderDrawerImage = (image,isFocused) => {
 ...
 const renderLabel = (name,isFocused) => {
    return (<Text style = {[styles.menuTextStyle,
         {color: isFocused ? colors.app_Blue : "rgba(62,62,20,1)", marginLeft:-10}]}>
             {name}</Text>)
 }
 const renderLabelMyApplications = (name,myApplicationsList,isFocused) => {
   
    const offerStatusObject = myApplicationsList.filter(el => el.offerStatus === 1)
    return (
        <View style={styles.viewLabelWithCount}>
 <Text style = {[styles.menuTextStyle,{color: isFocused ? colors.app_Blue : "rgba(62,62,20,1)",marginLeft:-10}]}>
        {name}
        </Text>
        {offerStatusObject.length>0 && 
        <View style={styles.viewCount}>
            <Text style={styles.textCount}>
                {offerStatusObject.length}
            </Text>
        </View>}
        </View>
   )
