...
import { kFormatter, gettUserData, NumberformatesunitUptoOneDecimal, fnum } from '../../../SupportingFIles/Utills';
...
class CreateCampaignForm extends Component {
  ...
  componentDidMount() {
    ...
    if (type === 'Edit') {
      ...
      const genderIndex = this.state.genderArray.findIndex(x => x.title === store.campainData.lookingInfluencerGender)
...
this.setAndAddUserCountry(type,store)
  ...
  setAndAddUserCountry = (type,store) =>{
   ...
   // console.log('selectedCountries:',this.state.selectedCountries)
    ...
    if (userCountryObj.length>0 && countriesData.filter(e => e.name === userCountryObj[0].name).length === 0) {
  ...
  getUserCountryObj = () =>{
    const userCountryName = this.props.route.params.type !== 'Edit'?this.props.AuthStore.userlocationcountry:""
    var userCountryObj = countriesJson.filter(function(country){
      return country.name === userCountryName
   })
   userCountryObj=userCountryObj.map(el=>{
  ...
  setCampaignType = (campainData) => {
    ...
    if (campainData.campaignType === "paid") {
      this.setState({ paid: true, sponsored: false, shoutout: false ,commissionBased:false,eventsAppearence:false,photoshootVideo:false })
    }
    else if (campainData.campaignType === "shoutout") {
      this.setState({ paid: false, sponsored: false, shoutout: true ,commissionBased:false ,eventsAppearence:false,photoshootVideo:false})
    }
    else if (campainData.campaignType === "sponsored") {
      this.setState({ paid: false, sponsored: true, shoutout: false ,commissionBased:false,eventsAppearence:false ,photoshootVideo:false})
    }else if (campainData.campaignType === "commissionBased") {
      this.setState({ paid: false, sponsored: false, shoutout: false,commissionBased:true,eventsAppearence:false ,photoshootVideo:false})
  ...
  render() {
    ...
    const store = this.props.CreateCampaignStore
    const { campainData, validationError, isLoading, categoriesArray, enabled } = this.props.CreateCampaignStore
      ...
      >
        <View style={{paddingHorizontal:metrics.dimen_27, flex: 1,}}>
            <Text style={[styles.textPopupTitle,
              ...
              {'Campaign Types & Examples'}</Text>

          <View style={{marginTop:20,marginVertical:10}}>
            <Text style={styles.textPopupTitle}>Paid Campaign</Text>
            <Text style={styles.textPopupDescription}>{"eg. $300 for Posting on Tiktok"}</Text>
          </View>
          <View style={{marginVertical:10}}>
            <Text style={styles.textPopupTitle}>{'Commission Based'}</Text>
            <Text style={styles.textPopupDescription}>eg. 30% commission for every sale</Text>
          </View>
          <View style={{marginVertical:10}}>
            <Text style={styles.textPopupTitle}>{'Events Appearance'}</Text>
            <Text style={styles.textPopupDescription}>eg. $500, Restaurant, 7PM - 10PM</Text>
          </View>
          <View style={{marginVertical:10}}>
            <Text style={styles.textPopupTitle}>{'Photo / Video Shoot'}</Text>
            <Text style={styles.textPopupDescription}>eg. $800 for 3 hours photoshoot</Text>
          </View>
          <View style={{marginVertical:10}}>
            <Text style={styles.textPopupTitle}>{'Sponsored'}</Text>
            <Text style={styles.textPopupDescription}>eg. Free review for brand products or service for review</Text>
          </View>
          <View style={{marginVertical:10}}>
            <Text style={styles.textPopupTitle}>{'Shoutout Exchange'}</Text>
            <Text style={styles.textPopupDescription}>eg. Free shoutout between 2 Instagram accounts</Text>
          </View>
        </View>
      </RBSheet>
        <MyImagePicker
        ...
        <ScrollView style={{ marginTop: 0 }} scrollEnabled={this.state.scrollEnabledView}
        ref='_scrollView'
        >
          <TouchableWithoutFeedback onPress={() =>{
            this.setState({show_country_modal:false,scrollEnabledView:true})
            Keyboard.dismiss()} }accessible={false}>
            <View style={{ justifyContent: 'space-between', flex: 1 }}>
              <View>

                <Text style={[styles.textFieldTitle, { marginTop: 20 }]}>{strings('Campaigns_title')}</Text>
                ...
                <Text style={[styles.textFieldTitle, { marginTop: metrics.dimen_15 }]}>{strings('Campaigns_decription')}</Text>
                ...
                />
                {validationError.CampaignDetailError ? <Text style={{ ...commonStyles.errorTextStyle, marginHorizontal: metrics.dimen_27 }}>{validationError.CampaignDetailError}</Text> : null}
...
<View style={{flexDirection:'row',marginHorizontal:metrics.dimen_27,marginTop:metrics.dimen_5}}>
<Text style={[styles.textFieldTitletype, { marginRight: metrics.dimen_10}]}>{strings("Type")}</Text>

  <TouchableOpacity style={{}}
                onPress={() => this.bottomSheet.open()}>
                <Text style={[styles.textFieldTitletype, { color:colors.app_Blue }]}>{strings("SeeExample")}</Text>
                ...
                <View style={styles.viewAccountTypeSelction}>
                  <TouchableOpacity style={styles.radioButtonContainer}
                    ...
                    <View style={[styles.radioButton, !this.state.paid && styles.radioButtonUnselected]}>
                      {
                        this.state.paid ?
                    ...
                    <Text style={[styles.textFieldTitle, styles.textSaving]}>Paid Campaign</Text>
                  </TouchableOpacity>
                  ...
                  <TouchableOpacity style={styles.radioButtonContainer}
                    ...
                    onPress={() => {
                      if (editType !== "Edit") {
                        store.setCampaignData({ campaignType: "commissionBased" })
                        this.setState({ paid: false, sponsored: false, shoutout: false,commissionBased:true,eventsAppearence:false,photoshootVideo:false })
                      }

                    }}>
                    <View style={[styles.radioButton, !this.state.commissionBased && styles.radioButtonUnselected]}>
                      {
                    ...
                    <Text style={[styles.textFieldTitle, styles.textSaving]}>Commission Based</Text>
                  </TouchableOpacity>
                  ...
                  <TouchableOpacity style={styles.radioButtonContainer}
                    ...
                    <View style={[styles.radioButton, !this.state.sponsored && styles.radioButtonUnselected]}>
                      {
                    ...
                    <Text style={[styles.textFieldTitle, styles.textSaving]}>{"Product Sponsor"}</Text>
                  </TouchableOpacity>
                  ...
                  <TouchableOpacity style={styles.radioButtonContainer}
                    ...
                    <View style={[styles.radioButton, !this.state.shoutout && styles.radioButtonUnselected]}>
                      {
                    ...
                    <Text style={[styles.textFieldTitle, styles.textSaving]}>Shoutout Exchange</Text>
                  </TouchableOpacity>
                ...
                {(this.state.paid || this.state.sponsored||this.state.commissionBased||this.state.eventsAppearence||this.state.photoshootVideo) &&
                  <Text style={[styles.textFieldTitle, { marginTop: metrics.dimen_15 }]}>
                    {this.state.paid ? strings('Campaigns_budget') : strings('Value')}
                  </Text>}
                {(this.state.paid || this.state.sponsored||this.state.commissionBased||this.state.eventsAppearence||this.state.photoshootVideo) &&
                  <View style={styles.campaignViewStyle}>
                    <Text style={[styles.inputTextFieldStyle, { color: colors.app_Blue, marginLeft: metrics.dimen_14 }]}>{this.state.commissionBased?"%":strings('USD')}</Text>
                    <TextInput style={[styles.inputTextFieldStyle, { width: '100%', height: metrics.dimen_46, backgroundColor: colors.clear, paddingLeft: metrics.dimen_5 }]}
                      placeholder={this.state.commissionBased?" " :'0.00 (minimum price $5)'}
                ...
                {(this.state.paid || this.state.sponsored||this.state.commissionBased||this.state.eventsAppearence||this.state.photoshootVideo) && validationError.priceError ? <Text style={{ ...commonStyles.errorTextStyle, marginHorizontal: metrics.dimen_27 }}>{validationError.priceError}</Text> : null}
                ...
                <TouchableOpacity style={styles.campaignViewStyle}
                  onPress={() => this.props.navigation.navigate('CampaignAddCategory', { isFromAdd: true })}>
                  <TextInput style={[styles.inputTextFieldStyle, { width: '100%', height: metrics.dimen_46, backgroundColor: colors.clear, paddingLeft: metrics.dimen_10 }]}
                ...
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: metrics.dimen_10, marginHorizontal: metrics.dimen_27, }}>
                  ...
                  {validationError.categoryError ? <Text style={{ ...commonStyles.errorTextStyle }}>{validationError.categoryError}</Text> : null}
                </View>
                <Text style = {styles.textFieldTitle}>{strings('Expiry_Date')}</Text>
                  <TouchableOpacity onPress = {()=>{store.setShowDatePicker(true)}}>
                  <View style = {[styles.campaignViewStyle,{justifyContent: 'space-between',}]}>
                    <Text style={{fontFamily: metrics.Lato_SemiBold,
        fontSize: metrics.getFontSize(14), color: 'rgba(62,62,70,1.0)', marginLeft: metrics.dimen_14}}>{store.campainData.storyDate.length > 0 ? store.campainData.storyDate  : strings('Select_Date')}</Text>
                    <Image source={images.CalendarIcon} style={{marginRight: metrics.dimen_10}}/>
                  </View>
                  {validationError.dateError ? <Text style={{...commonStyles.errorTextStyle,marginHorizontal: metrics.dimen_27}}>{validationError.dateError}</Text> : null}

                  </TouchableOpacity>

                  <Text style={styles.textFieldTitle}>{strings('Campaign_Countries')}</Text>
                <TouchableOpacity style={styles.campaignViewStyle}
                  onPress={() => this.setState({show_country_modal:true,scrollEnabledView:false})}>
                  <TextInput style={[styles.inputTextFieldStyle, { width: '100%', height: metrics.dimen_46, backgroundColor: colors.clear, paddingLeft: metrics.dimen_10 }]}
        ...
        <View style={styles.countryPickerboxWithShadow}>
                    <CountryPicker
                      ...
                      onClose={() => this.setState({ show_country_modal: false, scrollEnabledView: true })}
                      isMultiple={true}
                      withEmoji={true}
                      withFlag={true}
                      withFilter={true}
                      ...
                      onSelect={country => {
                        console.log('country onSelect: =================>>>>>>. ', country)
                        store.setCampaignData({ country: country.name, countryCode: country.cca2 })
                        let data = this.state.selectedCountries
                        data.push({ name: country.name, cca2: country.cca2 })

                        var result = data.reduce((unique, o) => {
                          if (!unique.some(obj => obj.name === o.name && obj.cca2 === o.cca2)) {
                            unique.push(o);
                          }
                          return unique;
                        }, []);
                        ...
                        this.setState({ show_country_modal: false, selectedCountries: result, scrollEnabledView: true })
                        store.deleteValidationError('countryError')
                      }}
                      onDone={(selectedCountries) => {
                        ...
                        console.log('in Form:', selectedCountries)
                    this.setState({ show_country_modal: false, selectedCountries: selectedCountries, scrollEnabledView:true})} }
              />
        </View>
       }
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: metrics.dimen_10, marginHorizontal: metrics.dimen_27, }}>
                  
                  {selectedCountries.length === 250 &&  this.renderCountryTag({name:'ANY'})
                  }
                  {selectedCountries.length < 250 && selectedCountries.map((item, index) => {
                    return this.renderCountryTag(item)
                  }
                  )}
                  {validationError.countryError ? <Text style={{ ...commonStyles.errorTextStyle }}>{validationError.countryError}</Text> : null}
                </View>




                <Text style={styles.textFieldTitle}>{strings('Influencers_Gender')}</Text>
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: metrics.dimen_10, marginHorizontal: metrics.dimen_27, }}>
                ...
                <Text style={styles.textFieldTitle}>{strings('Influencers_Age')}</Text>
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: metrics.dimen_10, marginHorizontal: metrics.dimen_27, }}>
                  ...
                  {this.state.ageArray.map((item, index) => {
                    ...
                    return (
                      <TouchableOpacity style={[styles.itemAgeGender, item.isSelected && styles.selectedItemAgeGender]}
                        onPress={() => this.selectMultipleAgeGender(item, 'age')}>
                        <Text style={[styles.textItemAgeGender, item.isSelected && styles.textSelectedItemAgeGender]}>
                          {ageLabel}</Text>
                      </TouchableOpacity>
                    )
                ...
                <Text style={styles.textFieldTitle}>{strings('Min_Followers')}</Text>
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: metrics.dimen_10, marginHorizontal: metrics.dimen_27, }}>

                  {this.state.followersArray.map((item, index) => {
                    return (
                      <TouchableOpacity style={[styles.itemAgeGender, item.isSelected && styles.selectedItemAgeGender]}
                        onPress={() => this.selectDeselectFollower(index, 'followers')}>
                        <Text style={[styles.textItemAgeGender, item.isSelected && styles.textSelectedItemAgeGender]}>
                          {item.title === "Any" ? "Any" : kFormatter(item.title)}</Text>
                      </TouchableOpacity>
                    )
                  }
                  )}
             ...
             <Text style={styles.textFieldTitle}>{strings('Platforms')}</Text>
            <TouchableOpacity 
            onPress={() =>this.state.showbutton? this.clearAllSocialtype():this.selectAllSocialtype()}>
            <Text style={[styles.textFieldTitle,{color:"#4E8BFE"}]}>{this.state.showbutton?strings('Clear_All'):strings("Select_All")}</Text>
          </TouchableOpacity>

               </View>
                <FlatList
                  style={{  marginTop: metrics.dimen_15, marginBottom: metrics.dimen_10, marginHorizontal: metrics.dimen_10,  }}
                  contentContainerStyle={{ flexGrow: 1 }} 
                  numColumns={5}
                  keyboardShouldPersistTaps={'handled'}
                  data={this.state.SocialMediaSelectArray}
                  keyExtractor={(item, index) => index.toString()}
                  renderItem={({ item,index }) => this.renderSocialIcon(item,index)}
                />
                {/* <View style = {styles.campaignViewStyle}>
            {this.state.genderArray.map((item, index) =>{
                        return(
                ...
                <Text style={styles.textFieldTitle}>{strings('Provide_Campaign_image')}</Text>
                <FlatList
          ...
          <Button
            ...
            {
              backgroundColor: enabled ? colors.app_Blue : 'rgba(192, 196, 204, 0.6)',
  ...
  removeCountry = (item) => {
    ...
    {
      var filteredArr = this.state.selectedCountries.filter(function(el) { return el.cca2 !== item.cca2; }); 
...
try {
 ...
 const index = this.state.SocialMediaSelectArray.findIndex(obj => obj.type.includes(nameArr[i]));
 console.log('index',index)
  ...
  this.props.CreateCampaignStore.setCampaignData({socialMediaTags:selectedMediatagtype})
...
if(agearray!==""&&agearray!==null)
{
 ...
 const index = this.state.ageArray.findIndex(obj => obj.minAge.toString().includes(minmax[0])&&obj.maxAge.toString().includes(minmax[1]));
 console.log("index==",index) 
 var arrNew = this.state.ageArray
  ...
  selectDeselectSocialtype = (item) => {
    const index = this.state.SocialMediaSelectArray.findIndex(obj => obj.type === item.type);
  console.log('index',index)
    var arrNew = this.state.SocialMediaSelectArray
    console.log('arrNew',arrNew)
  ...
  selectMultipleAgeGender = (item) => {
   ...
   this.props.CreateCampaignStore.setCampaignData({ minAge: minvalue, maxAge: maxvalue })
    this.props.CreateCampaignStore.setCampaignData({minMaxAge:selectedagedate})
    }else
    {
    const index = this.state.ageArray.findIndex(obj => obj.maxAge === item.maxAge);
 ...
 getArrayMin(array){
  ...
  selectAgeGender = (index, type) => {
    ...
    if (type === 'age') {
      ...
      store.setCampaignData({
        followercountcampaign: index === 1 ? "10000" : index === 2 ? "100000" : "1000"
      })
  ...
  renderSocialIcon = (item,index) => {
    return (
      <TouchableOpacity style={{ marginLeft: metrics.dimen_10,}}
        ...
        <View style={{flexDirection:'column',alignItems:'center',justifyContent:'center',margin:metrics.dimen_6 }}>

        <Image style={{height:metrics.dimen_45,width:metrics.dimen_45}} source={item.image}  />
       <Text style={{fontSize:metrics.dimen_9,fontFamily:metrics.Lato_SemiBold,color:colors.app_gray,textTransform:'capitalize'}}>{item.type}</Text>

        </View>
        
        <Image style={{position:'absolute',right:1, height:metrics.dimen_18,width:metrics.dimen_18}} source={item.isSelected?images.socialselect:images.socialnormal}  />
      </TouchableOpacity>
    )
  }
  renderGallery = (item) => {
  ...
  setuserdataandredirect() { 
    ...
    if (store.campainData.campaignDetails.length === 0) {
      this.props.CreateCampaignStore.setValidationData({ CampaignDetailError: strings('Enter_Campaign_Description') })
    }
    ...
    // }
      ...
      this.props.CreateCampaignStore.setValidationData({ categoryError: strings('Select_Campaign_Categories') })
      this.refs._scrollView.scrollTo(100); 
    }
    else  if (this.props.CreateCampaignStore.campainData.storyDate.length === 0) {
    ...
    }else if((this.state.paid || this.state.sponsored||this.state.eventsAppearence||this.state.photoshootVideo)&&store.campainData.pricePerInstaStory<5)
    {
      this.props.CreateCampaignStore.setValidationData({ priceError: strings('minimum_PostAmount') })
      this.refs._scrollView.scrollTo(100); 
    }
    else {
      let campaignImage = []
...
export default inject("CreateCampaignStore", "MyProfileStore","AuthStore")(observer(CreateCampaignForm))