...
import { gettUserData, kFormatter, convertCurrencybyCode, NumberformatesunitUptoOneDecimal, getUserId, getParams, generateFirebaseCampaignlink } from '../../../SupportingFIles/Utills';
...
class CampaignDetails extends Component {
  ...
  componentDidMount() {
    ...
    this.props.navigation.setOptions({
      headerRight: () => (this.props.AuthStore.isLogin &&
    ...
    gettUserData().then(data => {
      ...
      if (data !== undefined && data.ownerId !== undefined && data.ownerId !== null) {
  ...
  applyCampaignView = (data) => {
    return (
      <View style={{ paddingHorizontal: metrics.dimen_2 }}>
        <Text style={[styles.textPopupTitle,
        { alignSelf: 'center', fontSize: metrics.text_large, fontFamily: metrics.Lato_Bold }]}>
          {'Apply for Campaign'}</Text>


          {data.campaignType !== "commissionBased"&&data.campaignType !== "shoutout"&&data.campaignType !== "sponsored"?  <Text style={{ ...commonStyles.LatoSemiBold_Normal, color: 'rgba(62, 62, 70, 1)', marginTop: metrics.dimen_20, marginHorizontal: metrics.dimen_20 }}>{strings('enter_MyFee')+" *"}</Text>:null}

        {data.campaignType !== "commissionBased"&&data.campaignType !== "shoutout"&&data.campaignType !== "sponsored"?  <View style={[styles.TextInputStyle,  
      ]}>

      <Text style={[styles.inputTextFieldStyle, ]}>{strings('USD')}</Text>

      <TextInput 
      style={{ width: '100%', backgroundColor: colors.clear, paddingLeft: metrics.dimen_5,    height: metrics.dimen_45,
          }}
        placeholder={"250"}
        autoCapitalize="none"
        autoCorrect={false}
        ...
        onChangeText={(text) => {this.setUpdateMyBidFee(text)}}
      />

        </View>:null}
        {data.campaignType !== "commissionBased"&&data.campaignType !== "shoutout"&&data.campaignType !== "sponsored"&&this.state.enterFeesValidation&&<Text style={{ ...commonStyles.LatoSemiBold_Normal, color: colors.app_red, marginTop: metrics.dimen_2, marginHorizontal: metrics.dimen_20 }}>{this.props.CompaignsStore.MyBidFee===""?strings('Please_enter_myfee'):"Minimum fee amount should be $1"}</Text>}

    
        <Text style={{ ...commonStyles.LatoSemiBold_Normal, color: 'rgba(62, 62, 70, 1)', marginTop: metrics.dimen_20, marginHorizontal: metrics.dimen_20 }}>{strings('Message_To_Brand')}</Text>
  ...
  render() {
    ...
    const { jobAwarded, isLoading, showReport, jobStatus, campaignStatusLoading, campaignAppliedRemarkId, appliedRemarkData, campaignByRemarklist, campaignRemark, appliedForCampaign, MyBidFee,isPaymentReleased } = this.props.CompaignsStore


    var campaignCategoriesArray = data.campaignCategories !== undefined ? data.campaignCategories : []
    const imageUrl = data.profile?data.profile.avatarUrl == null || data.profile.avatarUrl === '' || data.profile.avatarUrl === 'NA' ? images.userPlaceholder : { uri: data.profile.avatarUrl } : ""
...
<Modal
          ...
          <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
            ...
            <View style={styles.Alert_Message}>
              <Text style={styles.Alert_Messagetext}>{strings('Please_update_missing_details_from_your_Profile_to_continue_applying_on_Collaborations_Jobs')}</Text>

              <Text style={styles.Alert_MessageMissingtext}>{"Missing details : "+this.state.RequiredParameter}</Text>
              ...
              <View style={{ flexDirection: 'row', padding: metrics.dimen_8 }}>
                ...
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                  onPress={()=>{ this.setState({alertVisibility:false}) }}
                >
                  <Text style={styles.TextStyleDoItLater}>{strings("Do_it_Later")}</Text>
                </TouchableOpacity>
        ...
        <RBSheet
          ...
          height={data.campaignType !== "commissionBased"&&data.campaignType !== "shoutout"&&data.campaignType !== "sponsored"?metrics.dimen_360:metrics.dimen_280}
        ...
        <RaiseDispute modalVisible={true}
          itemData={data !== [] ? data : null}

        />
        <CounterOffer modalVisible={this.state.showcounterOffer}
        ...
        <ActionSheet
          ...
          options={parseInt(this.state.ownerid, 10) !== parseInt(data.ownerId, 10) ? [strings('send_message'), strings('Raise_dispute'), strings('Report'), strings('Cancel')] : [strings('Raise_dispute'), strings('Report'), strings('Cancel')]}
          cancelButtonIndex={parseInt(this.state.ownerid, 10) !== parseInt(data.ownerId, 10) ? 3 : 2}
          // destructiveButtonIndex={0}
          onPress={(index) => {

            parseInt(this.state.ownerid, 10) !== parseInt(data.ownerId, 10) ? this.actionSheetActions(index, data) : this.actionSheetActionSelfProfile(index, data)
          }}
        ...
        <Report show={showReport} onClose={this.onCloseReport} title={data.campaignTitle} campaignData={data} fromUserReport={false} />
        ...
        >
          ...
          {data.profile !== undefined &&
            <TouchableOpacity onPress={() => this.OpenUserProfile(data.profile)}>
              <View style={styles.ProfileView}>
                <Image source={imageUrl ? imageUrl : images.userPlaceholder} 
                fallback
                ...
                <View style={{ marginTop: metrics.dimen_5, marginLeft: metrics.dimen_12 }}>
                  <Text numberOfLines={1} style={[styles.title]}>{data.profile.first + " " + data.profile.last}</Text>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                    <Text style={{ ...styles.addresTextStyle, width: '48%' }}>{data.profile.country}</Text>
          ...
          <View style={{ position: 'absolute', top: metrics.dimen_10, right: metrics.dimen_8, backgroundColor: data.endStoryPostDate >= currentdate ? colors.app_green : colors.app_red, borderRadius: metrics.dimen_2 }} >
        ...
        /> */}
          <View style={{ marginHorizontal: metrics.dimen_16 }}>
            <Text style={{ ...commonStyles.LatoBold_16, marginTop: metrics.dimen_16 }}>{data.campaignTitle}</Text>

            {data.comments && data.comments.length > 0 ? <TouchableOpacity
              onPress={() => {
                gettUserData().then(Userdata => {
                  this.props.navigation.navigate('UserComment', { campaignData: data, userProfileUrl: Userdata ? Userdata.avatarUrl : "",Userdata:Userdata })
                })

              }}
            >
              <Text style={{ fontFamily: metrics.Lato_Regular, fontSize: metrics.text_medium, marginTop: metrics.dimen_8, color: "#7A818A" }}>{"View all " + data.comments.length + " comments"}</Text>
            </TouchableOpacity> : null}

          


      {data.campaignType !== "commissionBased"&&data.campaignType !== "shoutout"&&data.campaignType !== "sponsored"&& jobStatus===1&&appliedRemarkData.offerStatus!==3?
     <CampaignOfferStatus
                    campaignData={data}
                    appliedRemarkData={appliedRemarkData}    
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: metrics.dimen_20 }}>
              ...
              {/* } */}
              <View style={{ alignItems: 'center', marginRight: -metrics.dimen_10, height: 25, justifyContent: 'center' }}>
                ...
                {data.remarks !== undefined && data.campaignType !== "shoutout" &&
                  <Text style={[styles.tagTextStyle]}>
                    {data.remarks.length > 0 ? `${campaignByRemarklist.length} ${strings('Applications')}` : `${strings('New_Listing')}`}
                  </Text>}
            ...
            {data.campaignType === "shoutout" ? null : <Text style={{ ...commonStyles.LatoBold_12, color: 'rgba(112, 129, 138, 1)', marginTop: metrics.dimen_15 }}>{data.campaignType !== "paid" ? strings('ProductValue') : strings("Campaigns_budget")}</Text>}
            {data.campaignType === "shoutout" ? null : <Text style={{ ...commonStyles.LatoSemiBold_Normal, color: 'rgba(62, 62, 70, 1)', marginTop: metrics.dimen_6 }}>{`${data.campaignType === "commissionBased" ? formatCurrency.format(data.campaignAmount) + " %" : convertCurrencybyCode(data.campaignAmountCurrency) + formatCurrency.format(data.campaignAmount)}`}</Text>}

            {data.profile !== undefined && data.profile.followers > 0 ? <Text style={{ ...commonStyles.LatoBold_12, color: 'rgba(112, 129, 138, 1)', marginTop: metrics.dimen_20 }}>{strings('My_Followers')}</Text> : null}
            {data.profile !== undefined && data.profile.followers > 0 ? <Text style={{ ...commonStyles.LatoSemiBold_Normal, color: 'rgba(62, 62, 70, 1)', marginTop: metrics.dimen_6 }}>{NumberformatesunitUptoOneDecimal(data.profile.followers)}</Text> : null}

            <Text style={{ ...commonStyles.LatoBold_12, color: 'rgba(112, 129, 138, 1)', marginTop: metrics.dimen_20 }}>{strings('Description_withouts')}</Text>
            <Text style={{ ...commonStyles.LatoSemiBold_Normal, color: 'rgba(62, 62, 70, 1)', marginTop: metrics.dimen_6 }}>{data.campaignDetails}</Text>

            {campaignCategoriesArray.length > 0 && <Text style={{ ...commonStyles.LatoBold_12, color: 'rgba(112, 129, 138, 1)', marginTop: metrics.dimen_20 }}>{strings('Category')}</Text>}
            <Text style={{ ...commonStyles.LatoSemiBold_Normal, color: 'rgba(62, 62, 70, 1)', marginTop: metrics.dimen_6, textTransform: 'capitalize' }}>
      ...
      {campaignCategoriesArray.map(item =>{
            ...
            {data.country ? <Text style={{ ...commonStyles.LatoBold_12, color: 'rgba(112, 129, 138, 1)', marginTop: metrics.dimen_20 }}>{strings('Location')}</Text> : null}
            {data.country ? <Text style={{ ...commonStyles.LatoSemiBold_Normal, color: 'rgba(62, 62, 70, 1)', marginTop: metrics.dimen_6 }}>{data.country}</Text> : null}
      ...
      <Text style = {{...commonStyles.LatoSemiBold_Normal, color: 'rgba(62, 62, 70, 1)', marginTop: metrics.dimen_6}}>{data.shipping ? strings('Yes'): strings('No')}</Text> */}
            ...
            <Text style={{ ...commonStyles.LatoBold_12, color: 'rgba(112, 129, 138, 1)', marginTop: metrics.dimen_20 }}>{strings('Preferred_gender')}</Text>
            <Text style={{ ...commonStyles.LatoSemiBold_Normal, color: 'rgba(62, 62, 70, 1)', marginTop: metrics.dimen_6, textTransform: 'capitalize' }}>{data.lookingInfluencerGender}</Text>

            <Text style={{ ...commonStyles.LatoBold_12, color: 'rgba(112, 129, 138, 1)', marginTop: metrics.dimen_20 }}>{strings('InfluencerAge')}</Text>
            <Text style={{ ...commonStyles.LatoSemiBold_Normal, color: 'rgba(62, 62, 70, 1)', marginTop: metrics.dimen_6, textTransform: 'capitalize' }}>{data.minAge===0&& data.maxAge===0?"Any":data.minAge + " - " + data.maxAge}</Text>


            {data.minimumFollowers > 0 && <Text style={{ ...commonStyles.LatoBold_12, color: 'rgba(112, 129, 138, 1)', marginTop: metrics.dimen_20 }}>{strings('minimum_followers')}</Text>}
            {data.minimumFollowers > 0 && <Text style={{ ...commonStyles.LatoSemiBold_Normal, color: 'rgba(62, 62, 70, 1)', marginTop: metrics.dimen_6 }}>
      ...
      {hashTagsArray.map(item =>{
            ...
            {data.campaignType !== "commissionBased"&&data.campaignType !== "shoutout" &&data.campaignType !== "sponsored"&& feesData!==null&&
       <FeesInfoPoup
      ...
      <View style={{height:metrics.dimen_60}}>
          ...
          <View style={{ height: Platform.OS === "ios" ? metrics.dimen_60 : metrics.dimen_70 }}></View>
        </ScrollView>

        <View style={{ justifyContent: 'center', alignContent: 'center', marginBottom: metrics.dimen_25 }}>
          {isLoading === false && campaignStatusLoading === false &&
            this.props.AuthStore.isLogin &&
            jobAwarded != null &&
            !jobAwarded &&
            (jobStatus === null || jobStatus === 2||isPaymentReleased===1) &&
            parseInt(this.state.ownerid, 10) !== parseInt(data.ownerId, 10) &&

            <TouchableOpacity activeOpacity={0.8} style={{ ...styles.applyForCampaignButton }}
              onPress={() =>
                this.openApplyJobBottomSheet()

              }
            >
              <Text style={{ ...commonStyles.LatoBold_14, color: 'white', textTransform: 'uppercase' }}>
                {strings('Apply_for_campaign')}
              </Text>
            </TouchableOpacity>
          }

          {isLoading === false && campaignStatusLoading === false && this.props.AuthStore.isLogin === false && jobAwarded != null && !jobAwarded && (jobStatus === null || jobStatus === 2) && parseInt(this.state.ownerid, 10) !== data.ownerId && <TouchableOpacity activeOpacity={0.8} style={{ ...styles.applyForCampaignButton }}
            onPress={() =>
          ...
          {this.props.AuthStore.isLogin && jobStatus === 1&&isPaymentReleased===0 && parseInt(this.state.ownerid, 10) !== data.ownerId &&
            <TouchableOpacity activeOpacity={0.8}
              style={{ ...styles.applyForCampaignButton }}
              onPress={() => this.showAlert(campaignAppliedRemarkId)}>
              <Text style={{ ...commonStyles.LatoBold_14, color: 'white', textTransform: 'uppercase' }} >{strings('Cancel_My_Application')}</Text>
  ...
  StatusUpdate = () => {
    const applicantDataToChange = JSON.parse(JSON.stringify(this.props.CompaignsStore.appliedRemarkData))
  ...
  counterOfferStatusUpdate = () => {
    ...
    getUserId().then(userId => {
      const applicantDataToChange = JSON.parse(JSON.stringify(this.props.CompaignsStore.appliedRemarkData))
      applicantDataToChange.offerAmount = this.props.CompaignsStore.counterAmount
      applicantDataToChange.isCounter = 1
      applicantDataToChange.counterBy = userId
      applicantDataToChange.lastOfferAmount = this.props.CompaignsStore.counterAmount
      this.props.CompaignsStore.setappliedRemarkData(applicantDataToChange)
  ...
  generatelinklink() {
    ...
    generateFirebaseCampaignlink(this.props.route.params.data.id).then(campaignlink => {
      ...
      [
        { text: strings('No'), onPress: () => console.log('No Pressed') },
        {
          text: strings('Yes'), onPress: () => {
            this.props.CompaignsStore.cancelOnAppliedJob(campaignAppliedRemarkId, this.props.route.params.data.id)
  ...
  checkUserUpdatedData = (applyData) => {
    gettUserData().then(data => {
      // if (data.avatarUrl && data.gender && data.email && data.first && data.last && data.country && data.bio && data.interests.length > 0 && data.instaUsername && data.facebookUsername && data.instaPerPost > 0 && data.facebookPerPost > 0) {
      if (data.gender && data.email && data.first && data.last && data.country && data.bio && data.instaUsername && data.instaPerPost > 0 && data.dob && data.mobile && data.phoneCode) {
  ...
  onApply = (applyData,data) => {
    if (this.state.profileupdate) {
      ...
      if(applyData.campaignType!=="commissionBased"&&applyData.campaignType!=="shoutout")
      {

        if(this.props.CompaignsStore.MyBidFee===""||this.props.CompaignsStore.MyBidFee<1)
        {
...
export default inject("CompaignsStore", "AuthStore", "MyProfileStore")(observer(CampaignDetails))
