...
import { gettUserData, showAlert, kFormatter, showFlashBanner,convertCurrencybyCode,generateFirebaseCampaignlink,NumberformatesunitUptoOneDecimal,getFeesAndCommission,getUserId } from '../../../SupportingFIles/Utills';
...
class MYCampaignDetails extends Component {
  ...
  checkBankInfo = () =>{
    gettUserData().then(userData => {
      ...
      if (userData.stripeBankAccountId !== null && userData.stripeBankAccountId !== "") {
...
openUserLike(item)
  ...
  render() {
    ...
    const isFromMyApplications = this.props.route.params !== undefined ? this.props.route.params.isFromMyApplications : false
    ...
    return (
      ...
      >
        ...
        <MarkAsComplete modalVisible={this.state.showMarkAsCompleted} 
...
<CounterOffer modalVisible={this.state.showcounterOffer} 
  ...
  {(isLoading || this.props.CreateCampaignStore.isLoading) && 
        ...
        {(!isLoading || !this.props.CreateCampaignStore.isLoading) &&  <ScrollView
       ...
       />
          ...
          <View style={{ marginHorizontal: metrics.dimen_16 }}>
              ...
              <Text style={{ ...commonStyles.LatoRegular_Medium, color: 'rgba(112, 129, 138, 1)',marginTop:metrics.dimen_12, }}>{strings('Posted_On') + " : " + Moment(data.createdAt).format('DD MMM, YYYY')}</Text>
          ...
          <Text style={{ ...commonStyles.LatoBold_16, marginTop: metrics.dimen_1 ,width:this.props.route.params.data.campaignStatus > 1 && !isFromMyApplications ?'65%':"90%" }}>{data.campaignTitle}</Text>

              {this.props.route.params.data.campaignStatus > 1 && !isFromMyApplications && <View style={styles.switchStyles} >
           ...
           {data.likes&&data.likes.length>0? 
            <TouchableOpacity style={{flexDirection:'row',marginTop:metrics.dimen_2}} onPress={() => {this.openUserLike(data)}}  >  
        ...
        <Text style = {{marginLeft:metrics.dimen_8, marginRight:metrics.dimen_15, fontFamily:metrics.Lato_Regular, fontSize: metrics.text_11, marginTop:Platform.OS === 'android' ?metrics.dimen_2:metrics.dimen_1 ,color:"#7A818A"}}>{data.likes.length+" likes"}</Text>
           </TouchableOpacity> :null} 
      {data.comments&&data.comments.length>0?<TouchableOpacity onPress={()=>{ this.openUserCommentBox(data)}}> 
     ...
     <Text style = {{marginLeft:metrics.dimen_8, marginRight:metrics.dimen_15, fontFamily:metrics.Lato_Regular, fontSize: metrics.text_11, marginTop:Platform.OS === 'android' ?metrics.dimen_2:metrics.dimen_1  ,color:"#7A818A"}}>{data.comments.length+" comments"}</Text>
  ...
  <View style={{flexDirection:'row', alignItems:'center',  paddingTop: metrics.dimen_12,}}>
...
<Image style={{alignSelf:'center', width:15, height:15, tintColor: colors.app_Blue}}
            ...
            {data.remarks !== undefined && data.remarks.length > 0 &&
              <TouchableOpacity style={styles.containerViewCampaignDetail}
                onPress={() => this.props.navigation.navigate('ApplicantList', { JobData: data })}
              >
                <Text style={styles.textCampaignDetail}>{strings('Hired_Influencers')}</Text>
              </TouchableOpacity>}
          
         { applicantData !== undefined&&applicantData.remarkStatus===1&&applicantData.offerStatus!==3&&data.campaignType !== "commissionBased"&&data.campaignType !== "shoutout"&&data.campaignType !== "sponsored"&&applicantData.offerAmount>0?
...
{/* </View>
    ...
    {data.campaignType==="shoutout"?null:   <Text style = {{...commonStyles.LatoBold_12, color: 'rgba(112, 129, 138, 1)', marginTop: metrics.dimen_20}}>{data.campaignType!=="paid"?strings('ProductValue'):strings("Campaigns_budget")}</Text>}
     {data.campaignType==="shoutout"?null:  <Text style = {{...commonStyles.LatoSemiBold_Normal, color: 'rgba(62, 62, 70, 1)', marginTop: metrics.dimen_6}}>{data.campaignType==="commissionBased"?`${data.campaignAmount+ " %"}` : `${convertCurrencybyCode(data.campaignAmountCurrency) + formatCurrency.format(data.campaignAmount)}` }</Text>}
      ...
      {data.profile !== undefined && data.profile.followers>0?   <Text style = {{...commonStyles.LatoSemiBold_Normal, color: 'rgba(62, 62, 70, 1)', marginTop: metrics.dimen_6}}>{NumberformatesunitUptoOneDecimal(data.profile.followers)}</Text>:null}
            ...
            <Text style={{ ...commonStyles.LatoBold_12, color: 'rgba(112, 129, 138, 1)', marginTop: metrics.dimen_20 }}>{strings('Description')}</Text>
      ...
      <Text style = {{...commonStyles.LatoSemiBold_Normal, color: 'rgba(62, 62, 70, 1)', marginTop: metrics.dimen_6}}>{data.country}</Text> */}
            ...
            {data.shipping && <Text style={{ ...commonStyles.LatoBold_12, color: 'rgba(112, 129, 138, 1)', marginTop: metrics.dimen_20 }}>{strings('Product_Shipped')}</Text>}
            {data.shipping && <Text style={{ ...commonStyles.LatoSemiBold_Normal, color: 'rgba(62, 62, 70, 1)', marginTop: metrics.dimen_6 }}>{data.shipping ? strings('Yes') : strings('No')}</Text>}
      ...
      Editing Campaign button should be disable after campaign Approval */}
        {this.props.route.params.data.campaignStatus > 0&&this.props.route.params.data.campaignStatus < 2 && !isFromMyApplications && <TouchableOpacity style={{ ...styles.applyForCampaignButton }}
          onPress={() => this.OpenUpdateCampaign()}
        >
          <Text style={{ ...commonStyles.LatoBold_16, color: 'white', textTransform:'uppercase' }}>{strings('Edit')}</Text>
        </TouchableOpacity>
        }

        {this.props.route.params.data.campaignStatus >=2 
        && applicantData === undefined
        && <View style={{ ...styles.bottomViewStyle, backgroundColor: colors.app_light_gray }}>
          <Text style={{ ...commonStyles.LatoBold_14, color: "#7A818A",  textTransform:'uppercase',marginTop:metrics.dimen_15 }}>{strings("approved")}</Text>
        </View>
        }

        {isFromMyApplications && applicantData.isMarkAsDone === 0 
        && applicantData.offerStatus === 2
          && applicantData.isPayment === 1 
        &&
          !this.state.hideMarkAsCompletedButton &&
          <TouchableOpacity style={{ ...styles.applyForCampaignButton }}
            onPress={() =>
              this.props.CompaignsStore.setMarkAsDonPopupStatus(true)
              //this.setState({showMarkAsCompleted:true})
            }
          >
            <Text style={{ ...commonStyles.LatoBold_14, color: 'white',textTransform:'uppercase' }}>{strings('Mark_As_Completed')}</Text>
          </TouchableOpacity>
        }
       ...
       {isFromMyApplications && applicantData.isMarkAsDone === 0 
        ...
        &&<TouchableOpacity style={{ ...styles.applyForCampaignButton }}
            onPress={() =>
              this.showAlert(applicantData,data)
...
<Image style={{alignSelf:'center', width:15, height:15, tintColor: colors.app_Blue}}
  ...
  getOfferStatusText = (applicantData) =>{
    if(applicantData.offerStatus === 2){
      var statusStr = `${strings('Offer_Accepted_Message_1')} $${formatCurrency.format(applicantData.offerAmount)}. `
      if (applicantData.isPayment === 1 && applicantData.isPaymentReleased === 1)
      {
      ...
      else if (applicantData.isPayment === 1 && applicantData.isPaymentReleased === 0)
      {
      ...
      return `${strings('Offer_Rejected_Message_1')} $${formatCurrency.format(applicantData.offerAmount)}`
    }
  }
  ...
  OpenUpdateCampaign() {
    ...
    this.props.CreateCampaignStore.getCampaigndatabyid(this.props.route.params.data.id)
    setTimeout(() => {
  ...
  checkBankAccount = (id, status, ownerId,store,applicantData) =>{
    gettUserData().then(userData => {
      console.log('user data value =', userData)
      if(userData.stripeAccountNumber === null || userData.stripeAccountNumber === ''){
        Alert.alert(
          "",
          ...
          [
            ...
            { text: "Add Bank", onPress: () => this.props.navigation.navigate('SettingStackScreen', { screen: 'BankInfoDetail' }),
            style: "cancel" }
          ],
          { cancelable: false }
        );
...
export default inject("CompaignsStore", "AuthStore", "CreateCampaignStore")(observer(MYCampaignDetails))
