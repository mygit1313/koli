...
class Social extends Component {
      ...
      headerTitle: () => (
        <View style={{ justifyContent: 'center', alignItems: 'center', color: "#1A1E24" }}>
          <Text style={{ fontFamily: metrics.Lato_Bold, fontSize: metrics.text_16, width: metrics.widthSize(700), textAlign: 'center',textTransform:'capitalize' }}
            numberOfLines={1}>
                 {this.props.route.params.Type==="Insta"?"Instagram":"Facebook"}
  ...
  render() {
    ...
    const { instaperpost, facebookperpost, instaUserName, followersCount, userExist, facebookUsername, isLoading } = store
    ...
    return (
      <View style={{ flex: 1, backgroundColor: 'white' }}>
        ...
        >
          ...
          <Text style={{ ...styles.signUpTextStyle, marginHorizontal: metrics.dimen_20, fontSize: metrics.text_xxl, marginTop: metrics.dimen_15 }}>{strings('Link_Social_Media')}</Text>

       
          {this.props.route.params.Type==="Insta"? <Text style={{ ...styles.signUpTextStyle, marginHorizontal: metrics.dimen_20, marginTop: metrics.dimen_15, fontWeight: metrics.LatoRegular }}>{strings('Instagram')}</Text>:null}
          {this.props.route.params.Type==="Insta"?     <View style={{ justifyContent: 'space-between', marginHorizontal: metrics.dimen_20, marginTop: metrics.dimen_10, flexDirection: 'row' }}>
            ...
            >
              <View style={
                [commonStyles.campaignViewStyle,
                  ...
                  isFromApplyJob && instaUserName.length === 0 && styles.emptyInputStyle, 
                 ]
                }>
                <View style={{ flexDirection: "row", marginLeft: metrics.dimen_10, marginTop: 2, alignItems: 'center' }}>
                  <Image source={images.instaIcon} style={{ width: metrics.dimen_20, height: metrics.dimen_20 }} />
            ...
            <View style={{ ...commonStyles.campaignViewStyle, width: '15%', alignItems: 'center', justifyContent: 'center' }}>
              <TouchableOpacity disabled={instaUserName === 0}
              ...
              >
                <Image source={images.trashIcon} style={{ tintColor: userExist || instaUserName.length > 0 ? colors.app_Blue : 'rgba(176, 176, 176, 1)' }} />
              </TouchableOpacity>
            </View>
          </View>:null}

          {this.props.route.params.Type==="Insta"? <Text style={{ ...styles.signUpTextStyle, marginHorizontal: metrics.dimen_20, marginTop: metrics.dimen_15 }}>
            {strings('Price_Per_Post')}
          </Text>:null}

          {this.props.route.params.Type==="Insta"?  <TextInput style={[styles.textInputStyle, isFromApplyJob && instaperpost == 0 && styles.emptyInputStyle,]}
...
{this.props.route.params.Type==="Fb"? <Text style={{ ...styles.signUpTextStyle, marginHorizontal: metrics.dimen_20, marginTop: metrics.dimen_15, fontWeight: metrics.LatoRegular }}>{strings('Facebook')}</Text>:null}
{this.props.route.params.Type==="Fb"? <View style={{ justifyContent: 'space-between', marginHorizontal: metrics.dimen_20, marginTop: metrics.dimen_10, flexDirection: 'row' }}>
            <TouchableOpacity style={{ width: '80%' }}
              onPress={() => this.doFacebookLogin()}
              disabled={facebookUsername.length > 0}
            >
              <View style={{ ...commonStyles.campaignViewStyle, justifyContent: 'space-between', alignItems: 'center' }}>
                <View style={{ flexDirection: "row", marginLeft: metrics.dimen_10, alignItems: 'center' }}>
                  <Image source={images.facebookLogo} style={{ width: metrics.dimen_20, height: metrics.dimen_20 }} />
                  <Text style={{ ...commonStyles.LatoSemiBold_Normal, color: colors.app_black, marginLeft: metrics.dimen_5 }}>{facebookUsername.length > 0 ? "@" + facebookUsername : strings('Link_Now')}</Text>
                </View>
                {/* <Text style={{...commonStyles.LatoSemiBold_Normal, color: colors.app_black, marginRight: metrics.dimen_10}}>{followersCount == 0 ? "" :  fnum(followersCount) + " " + strings('Followers')}</Text> */}
              </View>
            </TouchableOpacity>
            <View style={{ ...commonStyles.campaignViewStyle, width: '15%', alignItems: 'center', justifyContent: 'center' }}>
              ...
              >
                <Image source={images.trashIcon} style={{ tintColor: facebookUsername.length > 0 ? colors.app_Blue : 'rgba(176, 176, 176, 1)' }} />
              </TouchableOpacity>
            </View>
          </View>:null}

          {this.props.route.params.Type==="Fb"? <Text style={{ ...styles.signUpTextStyle, marginHorizontal: metrics.dimen_20, marginTop: metrics.dimen_15 }}>{strings('Price_Per_Post')}</Text>:null}
          {this.props.route.params.Type==="Fb"?  <TextInput style={{ ...styles.textInputStyle }}
            placeholder={strings('Enter_perpostgig')}
            placeholderTextColor='rgba(192,196,204,1)'
            keyboardType="number-pad"
  ...
  renderBottomSheet() {
    ...
    return (
      <RBSheet
        ...
        height={180}
        duration={170}>
        <Text style={{ ...commonStyles.LatoSemiBold_Normal, color: colors.app_black, marginTop: metrics.dimen_10, marginLeft: metrics.dimen_20 }}>{strings('UserName')}</Text>
        <TextInput
          ...
          autoCorrect={false}
        />
        <Button style={{ ...styles.addButtonStyle, margin: metrics.dimen_20,marginTop: metrics.dimen_5, borderColor: colors.app_Blue, height: metrics.dimen_46 }}
