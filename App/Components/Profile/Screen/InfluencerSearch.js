...
import { Numberformatesunit,getInfluencerSearchData,setInfluencerSearchData } from '../../../SupportingFIles/Utills'
...
class InfluencerSearch extends Component {
  ...
  render() {
    ...
    return (
      <View style={{ flex: 1, backgroundColor: 'white' }}>
        ...
        {store.isLoading===false&&store.InfluencerList.length<=0&&this.state.InfluencerSearchData.length>0? <View style={{flexDirection:"row", height:metrics.dimen_40,justifyContent:"space-between",alignContent:'center',alignItems:'center',backgroundColor:'rgba(248, 248, 248, 1)',paddingHorizontal:metrics.dimen_20}}>
        ...
        {store.InfluencerList.length<=0?<View style={{marginBottom:metrics.dimen_170}}> 
        <FlatList          
                  showsVerticalScrollIndicator={false}
                  numColumns={1}
                  data={this.state.InfluencerSearchData.sort((a, b) => b.id-(a.id))}
     ...
     {store.InfluencerList.length>0 &&  <FlatList
          style={{ flex: 1, }}
          ...
          onEndReached={() => {
            if (!store.end_curser&&store.hasNextPage>=store.offSetlength) {

              this.props.InfluencerSearchStore.getSearchInfluencerNextPage(store.updateSearch)

            }
          }}
          />}
        {store.InfluencerList.length === 0&&this.state.InfluencerSearchData.length===0 && this.NODataRender()}
  ...
  renderInfluencersearchItem = (item) => {
   ...
   let fullName=item.first?item.first.replace(/^"|"$/g, '')+" "+lastName:''
   const urlPic = item.avatarUrl
   const imageUrl = (urlPic === null || urlPic === 'NA' || urlPic === '') ? images.userPlaceholder : { uri: urlPic }
    return (
      <TouchableOpacity onPress = {()=> this.props.navigation.navigate('UserProfile',{UserData: item})}>
      ...
      >
        <View style={styles.ListMainView}>
          ...
          <View style={styles.datamainViewContainer}>
            ...
            <Text numberOfLines={1} style={[styles.subtitle]}>{item.instaUsername}</Text>

            {item.instaUsername?<View style={{ flexDirection: 'row', }}>
              <Text style={[styles.subtitle, { fontSize: metrics.text_medium }]} >{Numberformatesunit(item.followersCount)}</Text>
              <Image style={{ width: metrics.getW(15), height: metrics.getW(15), marginTop: metrics.dimen_5, margin: metrics.dimen_2 }} source={images.users}></Image>
        ...
        <View style={[styles.seprater,{marginVertical:metrics.dimen_15}]}></View>
      </View>
      </TouchableOpacity>
    )
  }
  ...
  UpdateSearchData(search){
    ...
    InfluencerSearchdata = influencersearchdata.filter(obj => (obj.name.toLowerCase()!==search.toLowerCase()))
    InfluencerSearchdata.push({id:InfluencerSearchdata.length+1, name:search });
    this.setState({InfluencerSearchData:InfluencerSearchdata})
...
searchcampaignfromRecentSearchData(item)
  ...
  deleteItemByName (name) {
    const filteredData = this.state.InfluencerSearchData.filter(item => item.name.toLowerCase()!== name.toLowerCase());
