...
class Interests extends Component {
  ...
  render() {
    ...
    <Loader loading={isLoading}/>
        ...
        <Text style = {{...styles.signUpTextStyle, marginHorizontal: metrics.dimen_20, marginTop: metrics.dimen_20, fontWeight: metrics.LatoRegular}}>{strings('IntrestInfo')}</Text>
        <View style = {{flexDirection: 'row', flexWrap: 'wrap', marginHorizontal: metrics.dimen_20, marginVertical:metrics.dimen_20}}>
        {store.interests.map(item =>{
            ...
            <TouchableOpacity style={[{
              ...
              backgroundColor: myInterests.includes(item) ? colors.app_Blue : colors.app_light_gray, 
              shadowColor: myInterests.includes(item) ?colors.app_Blue : colors.app_light_gray ,
              borderColor: myInterests.includes(item) ?colors.app_Blue : colors.app_light_black
              }]}
              onPress = {()=>{
                ...
                // {
                  if(myInterests.includes(item)){
                    ...
                    var filtered = myInterests.filter(function(value, index, arr){ return value !== item})
              ...
              >
              <Text style={{...commonStyles.LatoSemiBold_Normal, color: myInterests.includes(item) ? 'white' : colors.app_black}}>{item}</Text>
            </TouchableOpacity>
