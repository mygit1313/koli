...
import {Text,View,ScrollView,FlatList,Image,StyleSheet,TouchableOpacity,StatusBar,Dimensions} from 'react-native';
    ...
    // calculaterating.toFixed(1)
     return(
         ...
         <View style = {styles.container}>
             ...
             <View style={{flexDirection:'column',justifyContent:'center',alignItems:'center',borderBottomWidth:0.5,marginTop:metrics.dimen_8,borderColor: colors.app_light_gray }}>
                
                 <Text style = {styles.boldTextStyle}>{calculaterating?parseFloat(calculaterating).toFixed(1):""}</Text>
                        <StarRating
                        containerStyle = {{width:metrics.getW(100)}}
                            ...
                            />
                 <Text style = {[styles.title,{paddingVertical:10,}]}>{"based on "+userReviewlist.length + " Reviews"}</Text>
              ...
              >
                <View style={styles.ListMainView}>
                  ...
                  <View style={styles.datamainViewContainer}>
                    ...
                    <Text numberOfLines={1} style={[styles.subtitle]}>{item.profile.username ? "@" + item.profile.username : ""}</Text>
                  </View>
              <View style={{flexDirection:'row'}}>
                  <StarRating
                  disabled={true}
                  ...
                  fullStarColor={colors.RatingYellow}
                    />               
               <Text style = {{marginLeft:metrics.dimen_2, fontFamily:metrics.Lato_Bold,color:colors.RatingYellow,fontSize:metrics.text_medium}}>{item.reviewRating}</Text>
               </View>
...
export default inject('UserProfileStore', "AuthStore")(observer(UserReviewList))
