...
class BlockUser extends Component {
  ...
  handleSearch = text => {
    ...
    this.setState({updatedsearch:text})
       var filterdata = this.props.BlockUserStore.BlockUserList.filter(obj => obj.profile.first.toLowerCase().includes(formattedQuery) || obj.profile.last.toLowerCase().includes(formattedQuery))
    ...
    this.props.navigation.setOptions({
      ...
      headerRight: () => (
        <View style={{ marginRight: metrics.dimen_15, flexDirection: 'row', }}>
         {store.BlockUserList.length !== 0&& <TouchableOpacity onPress={() => this.SearchOnOf()} >
      ...
      {this.state.searchvisible===true? <View styles={{ flex: 1, marginHorizontal: metrics.dimen_10,marginTop:metrics.dimen_10 }}>
          <SearchBar
            ...
            onClear={() => {
              ...
              this.props.BlockUserStore.setblockUserFilterList(this.props.BlockUserStore.BlockUserList)
            }}
          />
        </View>:null}

        {store.blockUserFilterList.length > 0 ? <FlatList
    ...
    return (
      ...
      >
        <View style={styles.ListMainView}>
          ...
          <TouchableOpacity onPress={()=>this.showAlert(firstName,item.id)}
           style={styles.crossclick}>
            <FastImage
              style={styles.crossImage}
              source={images.crossRemove}
  ...
  nODataRender() {
    ...
    return (
      <View style={styles.noviewccontainer}>
        ...
        <Text style={styles.noblocksubtitle}>{strings("Anyone_you_block_willshowup_here")}</Text>
