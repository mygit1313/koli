...
import { View, Text, StyleSheet, FlatList, TouchableOpacity, Image, StatusBar, Platform, RefreshControl, Modal } from 'react-native';
...
import { gettUserData, time_ago, check_time_seconds } from '../../../SupportingFIles/Utills';
...
class Influencer extends Component {
    ...
    this.props.navigation.setOptions({
      headerRight: () => (
        <View style={{ marginRight: metrics.dimen_20, flexDirection: 'row' }}>
          <TouchableOpacity onPress={() => this.props.navigation.navigate('InfluencerSearch')}>
    ...
    return (
      <View style={{ flex: 1, backgroundColor: 'white' }}>
        ...
        {!store.isLoading && <FlatList
          refreshControl={
            <RefreshControl refreshing={store.refreshing} onRefresh={() => this.onRefresh()} />
          }
          ref={this.props.scrollRef}
          style={{ flex: 1, }}
          data={store.InfluencerList}
          showsVerticalScrollIndicator={false}
          renderItem={({ item, index }) => this.renderInfluencerItem(item, index)}
          numColumns={3}
          ...
          onEndReached={() => {
            if (!store.end_curser && store.hasNextPage >= store.offSetlength && !store.fetchingNextPageData) {
              this.props.InfluencerStore.getInfluencerNextPage()

            }
          }}
        />}
        {!store.isLoading && <View style={styles.bottomViewStyle}>
          <TouchableOpacity style={styles.requestButtonStyle}
            onPress={() => {
            ...
            <View style={{ flexDirection: 'row' }}>
              ...
              <Text style={[styles.textStyleNormalwhite]}>{strings('Sort')}</Text>
        ...
        {store.isLoading &&
          <FlatList
            ...
            // showsVerticalScrollIndicator={false}
            renderItem={({ item, index }) => this.renderPlaceHolderView(item, index)}
            numColumns={3}
            keyExtractor={(item, index) => index.toString()}
          />
        }
        <Modal
          visible={store.showInfluencerSortPopup}
          transparent={true}>
  ...
  renderInfluencerItem = (item, index) => {
    ...
    var imageUrl = (urlPic === null || urlPic === 'NA' || urlPic === '') ? images.KoliSquarePlaceholder : { uri: urlPic }
    return (
  ...
  renderPlaceholder = () => {
    return (<Image source={images.userPlaceholder} style={styles.imagePlaceholder}
    />)
