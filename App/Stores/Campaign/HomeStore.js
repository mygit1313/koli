...
import {
    getCampaigns, getMostAppliedCampaigns, getMostappliedrecent, PostCampaignLikes, getCampaignByLike, PostComment, getCampaignByComment, delUserLikeFromkList,getuserFollowList
...
class HomeStore {
    ...
    getAllCampaignWithPagination = (campaignType) => {
        ...
        if (!this.isRefreshing) {
            ...
            console.log('condition home getAllCampaignWithPagination:', JSON.stringify(filterWhere))
    ...
    getMostAppliedRecentCampaigns = (campaignType) => {
        ...
        getUserId().then(userId => {
            ...
            getMostappliedrecent(queryParam).then(response => {
                this.setRefreshing(false)
                const { status, data } = response
                if (status === 200) {
    ...
    getMostAppliedCampaignsList = (campaignType) => {
        ...
        getUserId().then(userId => {
            const queryParam = `${userId}&campaignType=${campaignType}`
            getMostAppliedCampaigns(queryParam).then(response => {
                this.setRefreshing(false)
                const { status, data } = response
                if (status === 200) {
    ...
    postCampaignLike = (campaignId) => {
        ...
        getCampaignByLike(param).then(response => {
            console.log("Campaign Like", response.data)
            if (response.data.length > 0) {
                const likedCampaign = response.data.some(el => el.ownerId === this.UserLoginID)

                console.log("likedCampaign1", likedCampaign)
                if (!likedCampaign) {
                    getUserId().then(userId => {
                        const paramlike = { ownerId: userId, campaignId: campaignId }
                        PostCampaignLikes(paramlike).then(responselike => {
                ...
                getUserId().then(userId => {
                    ...
                    PostCampaignLikes(params).then(postresponse => {
                        //  this.setisLoading(false)
    ...
    getLikeByCampaign = (campaignId) => {
        ...
        const param = { "where": { "campaignId": campaignId }, include: ["profile"] ,order: [ "createdAt DESC"]  }
        getCampaignByLike(param).then(response => {
            const { status, data } = response
            ...
            if (status === 200 && !data.error) {
                ...
                {                
                   var arrNew=[]
                    ...
                    const result = arrNew.map(({ownerId, isFollowed,createdAt,id,modifiedAt,profile}) => ({ownerId,createdAt:createdAt,id:id,modifiedAt:modifiedAt,profile:profile ,isFollowed: isFollowed|| this.FollowerUserlist.some(item => item.followUserId === ownerId)}))
    ...
    delUserLikeList = (campaignId) => {
        ...
        getCampaignByLike(param).then(response => {
            ...
            if (data.length > 0) {
                ...
                const likeData = response.data.filter(el => el.ownerId === this.UserLoginID)
    ...
    postCommentOnCampaign = (campaignId, comment) => {
        getUserId().then(userId => {
            const param = { ownerId: userId, campaignId: campaignId, commentText: comment }

            PostComment(param).then(response => {
    ...
    getCommentByCampaign = (campaignId) => {
        this.setisLoadingLikelist(true)
        const param = { "where": { "campaignId": campaignId }, include: ["profile"] ,order: [ "createdAt DESC"]  }
        getCampaignByComment(param).then(response => {
            this.setisLoadingLikelist(false)
            const { status, data } = response
    ...
    getUserFollowesList = () => {
        ...
        getuserFollowList(param).then(response => {
            const { status, data } = response
