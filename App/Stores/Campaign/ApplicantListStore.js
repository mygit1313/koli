...
import { getCampaignDetails, 
    ...
    releaseUserPayment ,
    SubmitReview} from '../../API/Campaign/ApiCampaign';
// import { showFlashBanner } from '../../SupportingFIles/Utills';
import { getUserDetail } from '../../API/Profile/User/ApiProfile';
import { strings } from '../../Locales/i18';
import { getUserId, showAlert, getAccessToken, showFlashBanner } from '../../SupportingFIles/Utills';
...
class ApplicantListStore{
    ...
    getApplicantList = (campaignId, responseCallback) =>{
        ...
        getCampaignDetails(campaignId).then(response =>{
    ...
    onRelaseCampaignAmount = (paramsToSend) =>{
        ...
        this.setisLoading(true)
         ...
         transferCampaignAmount(paramsToSend).then(response =>{
    ...
    onPayoutAmount = (userData) =>{
      ...
      payoutAmountToUser(paramsToSend).then(response =>{
    ...
    remarkShortlisted = (remarkID,Status,campaignid) =>{
      ...
      ShortListedRemark(remarkID,paramsToSend).then(response =>{
    ...
    initiatePaypalPayment = (param, responseCallback) => {
        ...
        getUserId().then(userId => {
            getAccessToken().then(accessToken => {
                ...
                checkoutPaypalUsingNonce(accessToken, dataParams).then(response => {
    ...
    releasePaymentPaypal = (param, responseCallback) => {
        ...
        getUserId().then(userId => {
            getAccessToken().then(accessToken => {
                ...
                releaseUserPayment(accessToken, dataParams).then(response => {
    ...
    submitReview = (param,campaignId) =>{
        //this.setisLoading(true)
        SubmitReview(param,campaignId).then(response =>{
            this.setisLoading(false)
            const {status, data} = response
