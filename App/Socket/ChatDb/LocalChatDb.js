...
export const getChat = (userId) => {
   ...
   return Realm.open({ schema: [ChatSchema, UserSchema],schemaVersion: 3, })
    .then(realm => {
        realmDB = realm
        console.log("Realm DB Location:",JSON.stringify(realm.objects('Chat')))
        // ...use the realm instance here
 ...
 export const getAllUser = (userId) => {
    ...
    return Realm.open({ schema: [ChatSchema,UserSchema], schemaVersion: 3, })
     .then(realm => {
        ...
        //  const userData = realm !== undefined ? realm.objects('User').filtered(`_id != "${userId}" DISTINCT(_id)`) : []
        // const userData = realm.objects('User').filter(e=>e._id!=userId)
        //  debugger;
 ...
 export const doMigration = () =>{
    Realm.open({
        ...
        migration: (oldRealm, newRealm) => {
            console.log('OldRealm====>',oldRealm,newRealm)
