...
import { Alert, Platform, Dimensions, Linking, PermissionsAndroid } from 'react-native'
...
export const getInfluencerSortType = async () => {
  const influencerSortType = await AsyncStorage.getItem(keys.influencerSortType);
...
export const setCampaignSearchData = (CampaignSearchData) => {
  AsyncStorage.setItem("CampaignSearchData", JSON.stringify(CampaignSearchData));
}
...
export const setInfluencerSearchData = (InfluencerSearchData) => {
  ...
  AsyncStorage.setItem("InfluencerSearchData", JSON.stringify(InfluencerSearchData));
}

export const getInfluencerSearchData = async () => {
  const InfluencerSearchData = await AsyncStorage.getItem("InfluencerSearchData");
...
export const fbLogin = async (callback) => {
  try {
    ...
    } else { console.log("fb login cancelled =", data) }
...
export function showAppUpdateAlert() {
  Alert.alert(
    ...
    [
      {
        text: "UPDATE",
        onPress: () => Linking.openURL('http://play.google.com/store/apps/details?id=co.koli.android')
...
export function kFormatter(num) {
  ...
  return Math.abs(num) > 1000 ? Math.sign(num) * ((Math.abs(num) / 1000).toFixed(1)) + 'k' : formatCurrency.format(num)
}
...
const hasLocationPermissionIOS = async () => {
  ...
  if (status === 'disabled' || status === 'denied') {
    Alert.alert(
      `To show influencer based on user location..`,
      '',
      [
